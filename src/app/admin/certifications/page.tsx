...
interface CertItem { id: string; name: string; role: string; order: number; }

export default function AdminCertificationsPage() {
    const [items, setItems] = useState<CertItem[]>([]);
    const [loaded, setLoaded] = useState(false);
    const [editingId, setEditingId] = useState<string | null>(null);
    const [editData, setEditData] = useState<Partial<CertItem>>({});

    useEffect(() => {
        const q = query(collection(db, "certifications"), orderBy("order"));
        const unsub = onSnapshot(q, (snap) => {
            setItems(snap.docs.map((d) => ({ id: d.id, ...d.data() } as CertItem)));
            setLoaded(true);
        });
        return unsub;
    }, []);

    const handleAdd = async () => { await addDocument("certifications", { name: "", role: "", order: items.length }); };
    const startEdit = (item: CertItem) => { setEditingId(item.id); setEditData({ ...item }); };
    const handleSave = async () => {
        if (!editingId) return;
        const { id, ...rest } = editData as any;
        await updateDocument("certifications", editingId, rest);
        setEditingId(null);
    };
                    ...
                    <Column key={item.id} padding="m" radius="l" border="neutral-medium" background="surface" gap="m">
                        {editingId === item.id ? (
                            <>
                                ...
                                <Input id={`role-${item.id}`} label="Role / Status" value={editData.role || ""}
