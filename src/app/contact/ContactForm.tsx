...
export function ContactForm({ title, description }: ContactFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({ message: "Something went wrong. Please try again.", variant: "danger" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Column maxWidth="s" gap="l">
      ...
      <UiText variant="body-default-l">{description}</UiText>
      <Flex as="form" direction="column" gap="l" onSubmit={handleSubmit}>
        ...
        <Textarea id="message" label="Message" value={message} onChange={(e) => setMessage(e.target.value)} required rows={5} />
